...
{
	if((in_binary_image.width != out_SDF.width) || (in_binary_image.height != out_SDF.height)) 
		reallocImage(in_binary_image.width, in_binary_image.height, out_SDF);
		
	SSDFGrid g1, g2;
	g1.width = in_binary_image.width;
	g1.height = in_binary_image.height;
	g1.p = (SSDFPoint*)malloc(sizeof(SSDFPoint)*g1.width*g1.height);
	
	g2.width = in_binary_image.width;
	g2.height = in_binary_image.height;
	g2.p = (SSDFPoint*)malloc(sizeof(SSDFPoint)*g2.width*g2.height);
	
	for(int y=0; y<in_binary_image.height; y++)
	{
		for(int x=0; x<in_binary_image.width; x++)
		{
			if(in_binary_image.ptr[y*in_binary_image.width+x] < 0.5)
			{
	...
	for(int y=0; y<in_binary_image.height; y++)
	{
		for(int x=0; x<in_binary_image.width; x++)
		{
			...
			out_SDF.ptr[y*in_binary_image.width+x] = dist;
		}
	}
...
{
	if((in_binary_image.width != out_DF.width) || (in_binary_image.height != out_DF.height)) 
		reallocImage(in_binary_image.width, in_binary_image.height, out_DF);
	...
	for(int y=0; y<in_binary_image.height; y++)
	{
		for(int x=0; x<in_binary_image.width; x++)
		{
			...
			double dist2 = sqrt(getPoint(g2, x, y).distSq());
			double dist = dist1 - dist2;
			out_DF.ptr[y*in_binary_image.width+x] = fabs(dist);
		}
	}
